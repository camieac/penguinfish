...
//System.out.println("Aggro Engaged");
	...
	public void hitPlayer(Collision collision, Player player, Game game) {
		...
		if (enemyX >= (player.getX() + player.getPlayerSize() - 5)
				&& enemyX <= (player.getX() + player.getPlayerSize())
				&& enemyY >= (player.getY() - enemySize)
				&& enemyY <= (player.getY() + player.getPlayerSize())) {
			directionX = Direction.EAST;
			hit = true;
		}

		// Enemy hits player on the left
		if (enemyX >= (player.getX() - enemySize)
				&& enemyX <= (player.getX() - enemySize + 5)
				&& enemyY >= (player.getY() - enemySize)
				&& enemyY <= (player.getY() + player.getPlayerSize())) {
			directionX = Direction.WEST;
			hit = true;
		...
		if (enemyY >= (player.getY() + player.getPlayerSize() - 5)
				&& enemyY <= (player.getY() + player.getPlayerSize())
				&& enemyX >= (player.getX() - enemySize)
				&& enemyX <= (player.getX() + player.getPlayerSize())) {
			directionY = Direction.SOUTH;
			hit = true;
		}
		...
		if (enemyY >= (player.getY() - enemySize)
				&& enemyY <= (player.getY() - enemySize + 5)
				&& enemyX >= (player.getX() - enemySize)
				&& enemyX <= (player.getX() + player.getPlayerSize())) {
			directionY = Direction.NORTH;
			hit = true;
	...
	public void moveEnemy(Player player,Game game) {
		if(aggroTimeRemaining > 0){
			doAggro(player.getX(), player.getY(), player.getPlayerSize(), aggroRadius, game);
			aggroTimeRemaining--;
		}
		// Move enemy to right
		if (directionX == Direction.EAST) {
